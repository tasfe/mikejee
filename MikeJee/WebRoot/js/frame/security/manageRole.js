$(function() {
	...
	$('#datagridTb').datagrid({
		...
		toolbar : [ {
			...
			handler : function() {
				...
				$("#winFrame").attr("src", "toAddRole.do?deptId="+deptId);
				$("#dd").show();
				$('#dd').dialog({
					title : "新建角色",
					height : 300,
			...
			handler : function() {
				...
				$("#winFrame").attr("src", "toUpdateRole.do?roleId="+roleId);
				$("#dd").show();
				$('#dd').dialog({
					title : "修改角色",
					height : 300,
			...
			handler : function() {
				...
				if (row) {
					...
					$.messager.confirm('提示', '数据删除不可恢复，您确认删除吗?', function(r){
						if (r){
							$.getJSON("delRole.do", { roleId: row.roleId },  function(data){
								if (data == 'success') {
			...
			handler : function() {
				...
				var roleId = rows[0].roleId;
				$("#winFrame").attr("src", "toRoleAuthorize.do?roleId="+roleId);
				$("#dd").css("padding","0");
				$("#dd").show();
				$('#dd').dialog({
					title : "角色授权",
	...
	//隐藏字段
	$('#datagridTb').datagrid('hideColumn', "roleId")
